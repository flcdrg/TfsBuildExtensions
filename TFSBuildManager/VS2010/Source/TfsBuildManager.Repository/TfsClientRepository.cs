...
namespace TfsBuildManager.Repository
{
    ...
    public class TfsClientRepository : IDisposable, ITfsClientRepository
    {
        ...
        public void GenerateBuildNotes(IEnumerable<IBuildDetail> buildDefinitions, IEnumerable<string> noteOptions, string selectedProject)
        {
            ...
            var wordDocumentGenerator = new Program(buildDetails, noteOptions, exclusions, this.workItemStore, vcs, tms, selectedProject);
        ...
        public IEnumerable<Branch> GetChildBranchObjectsForItem(string item)
        {
            ...
            {
                childBranches.Add(new Branch() { Name = r.Item, ServerPath = r.Item });
            }
        ...
        {
            string expandedDrop = p;
            foreach (string macroName in macros.Keys)
            {
                expandedDrop = CaseInsesitivReplace(expandedDrop, macroName, macros[macroName]);
            }
